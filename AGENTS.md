...
- Ordinary still images can be viewed in an optional `3D Photo` mode that estimates depth locally and uses clearly noticeable camera parallax during a deliberate drag. Keep the centered view faithful to the source, but avoid depth so restrained that the effect is difficult to perceive. Keep this separate from VR projection and stereo tags; opening a new image still defaults to the tagged or fallback VR180 / Mono view.
- `3D Photo` provides explicit `Look` and `Touch` tools. Touch mode is the default when entering 3D Photo: dragging locally pulls the touched area, releasing adds an elastic return, and tapping creates a short damped jiggle. Constrain deformation by both screen-space radius and estimated-depth similarity so cheeks, food, and other foreground subjects move without dragging the whole background. `T` toggles Look / Touch.
