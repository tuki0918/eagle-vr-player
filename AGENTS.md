...
- Loop playback is toggled from a transport button positioned immediately to the left of volume. It defaults to off and is disabled for still images. The icon shows the current state: repeat-off while disabled and the regular repeat icon while enabled, without using an active accent color.
